# Homework 2: Classification using neural networks (1 Hidden Layer)
    ...
    for j in range(numHLLen): # number of neurons in hidden layer 1
        ...
        for m in range(l2FactorLen): # l2 regularization weight
            ...
            weights = {
                ...
                'out': tf.Variable(tf.random_normal([numHL1, numClass]))
            }
            ...
            def neural_net_1layer(x):
                ...
                layerOut = tf.matmul(layer1, weights['out']) + biases['out']

                return layerOut

            # Construct model
            logits = neural_net_1layer(X)
            ...
            lossOp += tf.reduce_mean(l2FactorHL1*regularizer1)
...
print("Optimized configuration: learnRate = {:5.4f}, numHL1 = {:5d}, l2FactorHL1 = {:2.1f}, Accuracy = {:4.3f}".
      format(learnRateRange[ind[0]],numHLRange[ind[1]], l2FactorRange[ind[2]],validScore1L[ind]))
...
l2FactorHL1 = l2FactorRange[ind[2]]
