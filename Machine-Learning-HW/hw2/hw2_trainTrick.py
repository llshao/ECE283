        ...
        for k in range(numHLLen):  # number of neurons in hidden layer 2
            ...
            for m in range(l2FactorLen): # l2 regularization weight
                ...
                for n in range(l2FactorLen):  # l2 regularization weight
                    ...
                    # tf.Variable will be updated
                    weights = {
                        'h1': tf.Variable(tf.random_uniform([inputDim, numHL1])),
                        'h2': tf.Variable(tf.random_uniform([numHL1, numHL2])),
                        'out': tf.Variable(tf.random_uniform([numHL2, numClass]))
                    }

                    biases = {
                        'b1': tf.Variable(tf.random_uniform([numHL1])),
                        'b2': tf.Variable(tf.random_uniform([numHL2])),
...
#-----------------------------------------------------------------------------------------------------------------------
# scaling to zero mean and unit variance via empirical estimates across the data set
xTrainMean = np.mean(xTrain,axis=0)
xTrainVar = np.std(xTrain,axis=0)
print((xTrainMean,xTrainVar))
xTrain[:,0] = (xTrain[:,0]-xTrainMean[0])/xTrainVar[0]
xTrain[:,1] = (xTrain[:,1]-xTrainMean[1])/xTrainVar[1]
print((np.mean(xTrain,axis=0),np.var(xTrain,axis=0)))

xTestMean = np.mean(xTest,axis=0)
xTestVar = np.std(xTest,axis=0)
xTest[:,0] = (xTest[:,0]-xTestMean[0])/xTestVar[0]
xTest[:,1] = (xTest[:,1]-xTestMean[1])/xTestVar[1]
print((np.mean(xTest,axis=0),np.var(xTest,axis=0)))

xValidMean = np.mean(xValid,axis=0)
xValidVar = np.std(xValid,axis=0)
xValid[:,0] = (xValid[:,0]-xValidMean[0])/xValidVar[0]
xValid[:,1] = (xValid[:,1]-xValidMean[1])/xValidVar[1]
print((np.mean(xValid,axis=0),np.var(xValid,axis=0)))
